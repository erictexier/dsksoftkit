...
class version_db_context(namedtuple('versionDbcontext',
                                    "showobj entity is_shot")):
    __slots__ = ()
...
class VersionInfoDb(GenTree):
    ...
    VER_PATERN = re.compile(r"%sv[\d]*%s" % (os.sep, os.sep))
    VERP_PATERN = re.compile(r'\.v[\d]*\.')
    # prod for non publish, temp for base on environment only
    VERP = re.compile(r'v[\d]*')
    ...
    def reset(self):
        ...
        self.path_to_frame = ""  # full filename to the frames (as sequence)
        self.path_to_movie = ""  # full filename to the movie
        self.thumbnailurl = ""
        self.iconfile = ""
    ...
    def setdata_shotgun(self, arg):
        ...
        if self.path_to_frame is None:
            self.path_to_frame = ''

        self.path_to_movie = arg.get("sg_path_to_movie", "")
        if self.path_to_movie is None:
            self.path_to_movie = ''
        ...
        if m:
            ...
            if m:
                self.version_number = int(m.group()[2:-1])
                self.version_str = m.group()[1:-1]
            else:
                m = self.VERP.search(self.getName())
                if m:
    ...
    def get_current_media(self):
        if VersionInfoDb.PREF_FRAME is True:
            if self._useframe is True and self.path_to_frame != "":
                return self.path_to_frame
            if self._useframe is False and self.path_to_movie != "":
    ...
    def has_cut(self):
        if (self.first_frame_cut != self.first_frame or
                self.last_frame_cut != self.last_frame):
            return True
        return False
    ...
    def get_frame_range_cut(self):
        if self.first_frame_cut == self.last_frame_cut:
            return ""
        return "%d%s%d" % (self.first_frame_cut,
                           self.DASH, self.last_frame_cut)

    def get_frame_range_cut_rv(self):
        print((self.first_frame_cut == self.last_frame_cut))
        if self.first_frame_cut == self.last_frame_cut:
            return ""
        if (self.first_frame_cut == self.first_frame and
                self.last_frame_cut == self.last_frame):
            return ""
