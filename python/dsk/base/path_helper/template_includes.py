...
from dsk.base.path_helper.includes import resolve_include
...
def _get_includes(file_name, data):
    ...
    if dsk_constants.SINGLE_INCLUDE_SECTION in data:
        # single include section
        includes.append( data[dsk_constants.SINGLE_INCLUDE_SECTION] )

    if dsk_constants.MULTI_INCLUDE_SECTION in data:
        # multi include section
        includes.extend( data[dsk_constants.MULTI_INCLUDE_SECTION] )
...
def _process_template_includes_r(file_name, data):
    ...
    for ts in dsk_constants.TEMPLATE_SECTIONS:
        output_data[ts] = {}
...
def process_includes(file_name, data):
    ...
    # Would result in:
    #   bar/something/@/_bar_
    template_paths = resolved_includes_data[dsk_constants.TEMPLATE_PATH_SECTION]
    template_strings = resolved_includes_data[dsk_constants.TEMPLATE_STRING_SECTION] 
    ...
    for templates in [template_paths, template_strings]:
        for template_name, template_definition in templates.items():
            ...
            if not template_str:
                raise DevError("Invalid template configuration for '%s' - "
...
def _resolve_template_r(template_paths, template_strings, template_name, template_definition, template_type, template_chain = None):
    ...
    if template_key in visited_templates:
        raise DevError("A cyclic %s template was found - '%s' references itself (%s)" 
    ...
    if isinstance(template_definition, dict):
        ...
        template_str = template_definition
    if not template_str:
        raise DevError("Invalid template configuration for '%s' - it looks like the "
    ...
    for part in template_str_parts:
        ...
        for ref_part in ref_parts[1:]:
            ...
            if not ref_template:
                raise DevError("Failed to resolve template reference from '@%s' defined by "
