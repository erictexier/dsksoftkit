...
"""
Implements a caching mechanism to avoid loading the same yaml file multiple
times unless it's changed on disk.
"""
...
class CacheItem(object):
    ...
    def __init__(self, path, data=None, stat=None):
        ...
        :param stat:    The stat of the file on disk. If not provided,
                        an os.stat will be run and the result stored.
        :raises:        Dev.errors.DevUnreadableFileError: File stat failure.
        """
    ...
    def size_differs(self, other):
        ...
        :param other:   The CacheItem to test against.
        :raises:        TypeError: Given item is not a CacheItem.
        :returns:       bool, True if other is a different size on disk,
                        False if not.
        """
        if not isinstance(other, CacheItem):
            raise TypeError("Given item must be of type CacheItem.")
    ...
    def __eq__(self, other):
        if not isinstance(other, CacheItem):
            raise TypeError("Given item must be of type CacheItem.")
        return (other.stat.st_mtime ==
                self.stat.st_mtime and not self.size_differs(other))
...
class YamlCache(object):
    ...
    def invalidate(self, path):
        """
        Invalidates the cache for a given path. This is usually called when
    ...
    def _add(self, item):
        """
        Adds the given item to the cache in a thread-safe way. If the given
        item is older (by file mtime) than the existing cache data for that
        file then the already-cached item will be returned. If the item is
        identical in file mtime and file size to what's cached, the
        already-cached item will be returned. Otherwise the item will be added
        to the cache and returned to the caller. If the given item is added to
        the cache and it has not already been populated with the yaml data
        from disk, that data will be read prior to the item being added
        to the cache.
        ...
        try:
            ...
            if self.is_static:
                ...
                if cached_item and cached_item == item:
                    ...
                    # Load the yaml data from disk. If it's not already
                    # populated.
                    if not item.data:
                        self._populate_cache_item_data(item)
                    self._cache[path] = item
