...
class Template(object):
    ...
    def _apply_fields(self, fields, ignore_types=None, platform=None):
        ...
        if keys is None:
            raise DevError("Tried to resolve a path from the template %s and a set "
        ...
        for token in tokens:
            ...
            if token.startswith('['):
                ...
                if not re.search("{*%s}" % dsk_constants.TEMPLATE_KEY_NAME_REGEX, token): 
                    raise DevError("Optional sections must include a key definition.")
            ...
            if re.search("[\[\]]", token): 
                raise DevError("Square brackets are not allowed outside of optional section definitions.")
    ...
    def _apply_fields(self, fields, ignore_types=None, platform=None):
        ...
        if platform is None:
            ...
            if self._per_platform_roots is None:
                ...
                raise DevError("Template %s cannot resolve path for operating system '%s' - "
            ...
            if platform_root_path is None:
                ...
                raise DevError("Cannot resolve path for operating system '%s'! Please ensure "
                ...
                raise DevError("Cannot evaluate path. Unsupported platform '%s'." % platform)
...
def read_templates(pipeline_configuration):
    ...
    if dup_names:
        raise DevError("Detected paths and strings with the same name: %s" % str(list(dup_names)))
...
def make_template_paths(data, keys, all_per_platform_roots):
    ...
    for template_name, template_data in templates_data.items():
        ...
        if "/" not in definition:
            raise DevError("The template %s (%s) does not seem to be a valid path. A valid "
        ...
        if root_path is None:
            raise DevError("Undefined Shotgun storage! The local file storage '%s' is not defined for this "
...
def _conform_template_data(template_data, template_name):
    ...
    if isinstance(template_data, basestring):
        ...
        raise DevError("template %s has data which is not a string or dictionary." % template_name)
...
def _process_templates_data(data, template_type):
    ...
    for template_name, template_data in data.items():
        ...
        if template_type == "path":
            if "root_name" not in cur_data:
                cur_data["root_name"] = dsk_constants.PRIMARY_STORAGE_NAME
    ...
    if dups_msg:
        raise DevError("It looks like you have one or more "
