...
import yaml
from authentication import user_impl

from .util import login
from .util import shotgun_entity
from .util import shotgun
from . import constants
from dsk.base.resources.dsk_errors import DskError, DskContextDeserializationError
...
class Context(object):
    ...
    objects are launch a toolkit engine via the :meth:`dsk.platform.start_engine`
    ...
    def __init__(self,
        ...
        methods :meth:`Dsk.context_from_entity`, 
                 meth:`Dsk.context_from_entity_dictionary`
                 meth:`Dsk.context_from_path`.
        """
        self.__tk = tk
    ...
    def __repr__(self):
        ...
        msg.append("  Department: %s" % str(self.__department))
    ...
    def as_template_fields(self, template, validate=False):
        ...
        context fields required by the template and if it can't then a :class:`DskError` will be raised.
        Example::
        ...
        :param validate:    If True then the fields found will be checked to ensure that all expected fields for
                            the context were found.  If a field is missing then a :class:`DskError` will be raised
        :returns:           A dictionary of template files representing the context. Handy to pass to for example
                            :meth:`Template.apply_fields`.
        :raises:            :class:`DskError` if the fields can't be resolved for some reason or if 'validate' is True
        ...
        if isinstance(template, TemplatePath):
            ...
            if self.entity and len(self.entity_locations) == 0:
                ...
                raise DskError("Cannot resolve template data for context '%s' - this context "
        ...
        if validate:
            # check that all context template fields were found and if not then raise a DskError
            missing_fields = []
            ...
            if missing_fields:
                raise DskError("Cannot resolve template fields for context '%s' - the following "
    ...
    def serialize(self, with_user_credentials=True):
        ...
        if with_user_credentials:
            ...
            if user:
                ...
                # run as.
                data["_current_user"] = user_impl.serialize_user(user)
    ...
    def deserialize(cls, context_str):
        ...
        try:
            ...
            raise DskContextDeserializationError(str(e))
        ...
        if user_string:
            ...
            user = user_impl.deserialize_user(user_string)
    ...
    def _fields_from_shotgun(self, template, entities, validate):
        ...
        :param validate: If True, missing fields will raise a DskError.

        :returns: Dictionary of field values extracted from Shotgun.
        :rtype: dict

        :raises DskError: Raised if a key is missing from the entities list when ``validate`` is ``True``.
        """
        fields = {}
        ...
        for key in template.keys.values():
            ...
            if key.shotgun_field_name:
                ...
                if not key.shotgun_entity_type in entities:
                    if validate:
                        raise DskError("Key '%s' in template '%s' could not be populated by "
                ...
                if cache_key in self._entity_fields_cache:
                    ...
                    if not result:
                        ...
                        raise DskError("Could not retrieve Shotgun data for key '%s' in "
                    ...
                    if value is None:
                        ...
                        if not key.validate(processed_val):                    
                            raise DskError("Template validation failed for value '%s'. This "
