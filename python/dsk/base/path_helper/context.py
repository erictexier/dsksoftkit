...
from dsk.base.resources.dsk_errors import DevError, DskContextDeserializationError
from dsk.base.db_helper.path_cache import PathCache
from dsk.base.path_helper.template import TemplatePath
...
class Context(object):
    ...
    def as_template_fields(self, template, validate=False):
        ...
        context fields required by the template and if it can't then a :class:`DevError` will be raised.
        Example::
        ...
        :param validate:    If True then the fields found will be checked to ensure that all expected fields for
                            the context were found.  If a field is missing then a :class:`DevError` will be raised
        :returns:           A dictionary of template files representing the context. Handy to pass to for example
                            :meth:`Template.apply_fields`.
        :raises:            :class:`DevError` if the fields can't be resolved for some reason or if 'validate' is True
        ...
        if validate:
            # check that all context template fields were found and if not then raise a DevError
            missing_fields = []
    ...
    def deserialize(cls, context_str):
        ...
        try:
            ...
            raise DevContextDeserializationError(str(e))
    ...
    def _fields_from_shotgun(self, template, entities, validate):
        ...
        :param validate: If True, missing fields will raise a DevError.

        :returns: Dictionary of field values extracted from Shotgun.
        :rtype: dict

        :raises DevError: Raised if a key is missing from the entities list when ``validate`` is ``True``.
        """
        fields = {}
...
def _from_entity_type_and_id(tk, entity, source_entity=None):
    ...
    if entity_type is None:
        raise DevError("Cannot create a context from an entity type 'None'!")

    if entity_id is None:
        raise DevError("Cannot create a context from an entity id set to 'None'!")
...
def _from_entity_dictionary(tk, entity_dictionary, source_entity=None):
    ...
    if "type" not in entity_dictionary:
        raise DevError("Cannot create a context without an entity type!")
    if "id" not in entity_dictionary:
        raise DevError("Cannot create a context without an entity id!")
...
def _task_from_sg(tk, task_id, additional_fields=None):
    ...
    if not task:
        raise DevError("Unable to locate Task with id %s in Shotgun" % task_id)
    ...
    for path in paths:
        ...
        if curr_entity is None:
            ...
            raise DevError(
                ...
                % (curr_path, entity_type, entity_id, dsk_constants.SUPPORT_EMAIL)
            )
        ...
        for key, value in path_fields.items():
            if key in unique_fields and value != unique_fields[key]:
                ...
                if key == entity["type"]:
                    ...
                    raise DevError(msg % (str(entity), str(cur_template)))
                else:
