...
All Toolkit logging is written into a ``dsk.*`` logging
...
- All core logging is written to the ``sgtk.base`` logger.

Below is a simple log hierarchy to illustrate what this might look like in practice.

.. code-block:: text

    dsk                                              Root point for all Toolkit logging
     |
     |- base                                          Root point for the Core API
     |   |
...
class LogManager(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not cls.__instance:
            ...
            instance._root_logger = logging.getLogger(dsk_constants.ROOT_LOGGER_NAME)

            # check the TK_DEBUG flag at startup
            # this controls the "global debug" state
            # in the log manager
            if dsk_constants.DEBUG_LOGGING_ENV_VAR in os.environ:
                log.debug("%s environment variable detected. Enabling debug logging." 
                          % dsk_constants.DEBUG_LOGGING_ENV_VAR)
    ...
    def get_logger(log_name):
        """
        Generates standard logger objects for Dsk.
        """
    ...
    def log_timing(func):
        ...
        ``dsk.stopwatch`` logger::

            [DEBUG dsk.stopwatch.module] my_shotgun_publish_method: 0.633s
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                timing_logger = logging.getLogger("%s.%s" % (dsk_constants.PROFILING_LOG_CHANNEL, func.__module__))
    ...
    def _set_global_debug(self, state):
        ...
        if not state:
            ...
            if dsk_constants.DEBUG_LOGGING_ENV_VAR in os.environ:
                ...
                del os.environ[dsk_constants.DEBUG_LOGGING_ENV_VAR]
        ...
        # log notifications
        if self._global_debug:
            log.debug("Debug logging enabled. To permanently enable it, "
                      "set the %s environment variable." % dsk_constants.DEBUG_LOGGING_ENV_VAR)
            ...
            log.debug("Setting %s in the environment for this session. This "
                      "ensures that subprocesses spawned from this process will "
                      "inherit the global debug logging setting from this process.",
                      dsk_constants.DEBUG_LOGGING_ENV_VAR)
            os.environ[dsk_constants.DEBUG_LOGGING_ENV_VAR] = "1"
    ...
    def log_folder(self):
        ...
        """
        if self._std_file_handler_log_file:
            return os.path.dirname(self._std_file_handler_log_file)
        return None
    ...
    def initialize_custom_handler(self, handler=None):
        ...
        .. note:: If you want to display log messages inside a DCC,
                  implement :meth:`~dsk.platform.Engine._emit_log_message`.
        ...
        if handler is None:
            ...
            # example: [DEBUG sgtk.log] message message
    ...
    def uninitialize_base_file_handler(self):
        ...
        log.debug("Tearing down existing log handler '%s' (%s)" % (base_log_file, 
                                                                   self._std_file_handler))
    ...
    def initialize_base_file_handler(self, log_name):
        """
        Create a file handler and attach it to the dsk base logger.
        ...
        When you start an engine via the :meth:`dsk.platform.start_engine` method,
        ...
        return self.initialize_base_file_handler_from_path(
            os.path.join(self.log_folder,"%s.log" % FileSystemUtils.create_valid_filename(log_name))
            )
    ...
    def initialize_base_file_handler_from_path(self, log_file):
        ...
        log.debug("Switching file based std logger from '%s' to '%s'.", previous_log_file, log_name)
        ...
        FileSystemUtils.ensure_folder_exists(log_folder)
        ...
        self._std_file_handler = handler_factory(log_file,
        ...
        # 2016-04-25 08:56:12,413 [44862 DEBUG sgtk.log] message message
...
#
# retrieve top most logger in the dsk hierarchy
dsk_root_logger = logging.getLogger(dsk_constants.ROOT_LOGGER_NAME)
...
# explicitly attach a log handler to the dsk top level
# logger (or any of its child loggers).
dsk_root_logger.propagate = False
...
dsk_root_logger.setLevel(logging.DEBUG)
#
...
dsk_root_logger.addHandler(NullHandler())
