...
class MsgNameChange(MsgArg):
    def __init__(self,widgetFrom=None,item=None,path=None,newName=None,which=0):
...
class MsgArgTimeChange(MsgArg):
    def __init__(self,widgetFrom,group,nodeTime,time,doEvaluate,doAllDirty):
...
class MsgArgConnection(MsgArg):
    def __init__(self,widgetFrom,group,fromName,toName,fromOutput,toInput):
...
class MsgArgGrid(MsgArg):
    def __init__(self,widgetFrom,group,xmin,ymin,xsize,ysize,xunit,yunit):
...
class MsgArgCreateNode(MsgArg):
    def __init__(self,widgetFrom,group,nodeModule,nodeName,nodeType):
