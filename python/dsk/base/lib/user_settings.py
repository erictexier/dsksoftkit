...
if sys.version_info[0] == 2:
    from ConfigParser import SafeConfigParser as aconfigparser
elif sys.version_info[0] >= 3:
    from configparser import ConfigParser as aconfigparser

from dsk.base.utils.platform_utils import LocalFileStorageManager
from dsk.base.lib.system_settings import SystemSettings
from dsk.base.resources.dsk_errors import EnvironmentVariableFileLookupError, DskError
# logging
from dsk.base.lib.log_manager import LogManager
logger = LogManager.get_logger(__name__)
# for easy logging
from dsk.base.resources.dsk_constants import ROOT_NAME,ROOT_NAME_ENV

setting_format="""
[Login]
default_site:erictexier@eclecticstudionet.com
default_login=
default_proxy=
...
class UserSettings(Singleton):
    ...
    def _init_singleton(self):
        ...
        logger.debug("System proxy: %s" % self._to_display_value(proxy))
    ...
    def get_setting(self, section, name):
        ...
        value = os.path.expanduser(os.path.expandvars(self._user_config.get(section, name)))
        return value.strip()
    ...
    def _compute_config_location(self):
        ...
        Retrieves the location of the ``config.ini`` file. It will look in multiple locations:
            ...
            - The ``DSK_PREFERENCES_LOCATION`` environment variable.
            - The ``DSK_DESKTOP_CONFIG_LOCATION`` environment variable.
            - The Shotgun folder.
        ...
        if ROOT_NAME_ENV in os.environ:
            alocation = LFSM.base_application_location(ROOT_NAME,
                                                       logfile = "%s_desktop.log" % ROOT_NAME.lower(),
                                                       is_user = True)
        else:
            alocation = LFSM.base_application_location(ROOT_NAME,
                                                       logfile = "%s_desktop.log" % ROOT_NAME.lower(),
                                                       alternative = ROOT_NAME_ENV,
                                                       is_user = False)

        self.data_location = alocation
        default_location = os.path.join(alocation.pref_dir_def,'dsk.ini')

        # This is the complete list of paths we need to test.
        file_locations = [self._evaluate_env_var("DSK_PREFERENCES_LOCATION"),
                          self._evaluate_env_var("DSK_CONFIG_LOCATION"),
                          default_location]
    ...
    def _get_filtered_proxy(self, proxy):
        ...
        if proxy and "@" in proxy:
            ...
            return ("<your credentials have been removed for security reasons>@%s" % 
                    proxy.rsplit("@", 1)[-1])
        else:
            return proxy
    ...
    def _to_display_value(self, value):
        ...
        :returns: If None, returns ``<missing>``. 
                  If an empty string, returns ``<empty>`. 
