...
from dsk.base.conn_helper.authentication.console_authentication import ConsoleLoginHandler, ConsoleRenewSessionHandler
from dsk.base.conn_helper.authentication.ui_authentication import UiAuthenticationHandler
...
def _get_ui_state():
    ...
    """
    if QtT and QtT.QtWidgets.QApplication.instance() is not None:
        return True
    else:
        return False
...
def renew_session(user):
    ...
    if has_ui:
        authenticator = UiAuthenticationHandler(is_session_renewal=True, 
                                                session_metadata=user.get_session_metadata())
    else:
        authenticator = ConsoleRenewSessionHandler()
    SessionRenewal.renew_session(user, authenticator)


def authenticate(default_host, default_login, http_proxy, fixed_host, asession):
    """
    ...
    return authenticator.authenticate(default_host, default_login, http_proxy,asession)
