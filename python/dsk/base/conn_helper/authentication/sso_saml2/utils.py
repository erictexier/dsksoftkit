...
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import set_logger_parent
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_saml_claims_expiration
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_session_expiration
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import _decode_cookies
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import _get_shotgun_user_id
...
def _get_site_infos(url, http_proxy=None):
    ...
    try:
        ...
        if url not in INFOS_CACHE or ((time.time() - INFOS_CACHE[url][0]) > INFOS_CACHE_TIMEOUT):
            ...
            infos = shotgun_api3.Shotgun(url,
                                         session_token="dummy",
                                         connect=False,
                                         http_proxy=http_proxy).info()
...
def _get_user_authentication_method(url, http_proxy=None):
    ...
    if "user_authentication_method" in infos:
        get_logger().debug("User authentication method for %s: %s",
                           url,
...
def is_unified_login_flow_enabled_on_site(url, http_proxy=None):
    ...
    if "unified_login_flow_enabled" in infos:
        get_logger().debug("unified_login_flow_enabled for %s: %s",
                           url,
