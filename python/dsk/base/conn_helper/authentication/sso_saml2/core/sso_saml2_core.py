...
from dsk.base.conn_helper.authentication.sso_saml2.core.authentication_session_data import AuthenticationSessionData

from dsk.base.conn_helper.authentication.sso_saml2.core.errors import SsoSaml2MissingQtCore
from dsk.base.conn_helper.authentication.sso_saml2.core.errors import SsoSaml2MissingQtGui
from dsk.base.conn_helper.authentication.sso_saml2.core.errors import SsoSaml2MissingQtNetwork
from dsk.base.conn_helper.authentication.sso_saml2.core.errors import SsoSaml2MissingQtWebKit


from dsk.base.conn_helper.authentication.sso_saml2.core.utils import _decode_cookies
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import _encode_cookies
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import _sanitize_http_proxy
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_csrf_key
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_csrf_token
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_logger
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_saml_claims_expiration
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_session_id
from dsk.base.conn_helper.authentication.sso_saml2.core.utils import get_user_name
...
class SsoSaml2Core(object):
    ...
    def __init__(self, window_title="Web Login", qt_modules=None):
        ...
        if isinstance(qt_modules,dict):
            QtCore = self._QtCore = qt_modules.get('QtCore')  # noqa
            QtGui = self._QtGui = qt_modules.get('QtGui')  # noqa
            QtWidgets = self._QtGui = qt_modules.get('QtWidgets')  # noqa
            QtNetwork = self._QtNetwork = qt_modules.get('QtNetwork')  # noqa
            QWebPage = self._QWebPage = qt_modules.get('QWebPage')  # noqa
            QWebSettings = self._QWebSettings = qt_modules.get('QWebSetting')  # noqa            

        else:
            QtCore = self._QtCore = qt_modules.QtCore
            QtGui = self._QtGui = qt_modules.QtGui  
            QtWidgets = self._QtWidgets = qt_modules.QtWidgets
            QtNetwork = self._QtNetwork = qt_modules.QtNetwork
            QWebPage = self._QWebPage = qt_modules.QWebPage
            QWebView = self._QWebView = qt_modules.QWebView
            # hack for now should be carry with QtT
            if qt_modules.which in "PySide2":
                from PySide2.QtWebKitWidgets import QWebSettings
            elif qt_modules.which in "PyQt5":
                from PySide2.QtWebKitWidgets import QWebSettings
            elif qt_modules.which in "PyQt4":
                from PyQt4.QtWebKit import QWebSettings
            elif qt_modules.which in "PySide":
        ...
        class TKWebPage(QWebPage):
            ...
            def acceptNavigationRequest(self, frame, request, n_type): # noqa
                ...
                if frame is None and n_type == QWebPage.NavigationType.NavigationTypeLinkClicked:
                    QtGui.QDesktopServices.openUrl(request.url())
                    return False
                # Otherwise we accept the default behaviour.
                return QWebPage.acceptNavigationRequest(self, frame, request, n_type)
        ...
        self._view = QWebView(self._dialog)
        ...
        self._view.page().action(QWebPage.Reload).setVisible(False)
        ...
        if self._logger.level == logging.DEBUG or "SHOTGUN_SSO_DEVELOPER_ENABLED" in os.environ:
            ...
            QWebSettings.globalSettings().setAttribute(QWebSettings.WebAttribute.DeveloperExtrasEnabled,
                                                       True)
            QWebSettings.globalSettings().setAttribute(QWebSettings.WebAttribute.LocalStorageEnabled,
                                                       True)
    ...
    def on_dialog_closed(self, result):
        ...
        QtWidgets = self._QtQWidgets  # noqa
