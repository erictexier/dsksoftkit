...
from dsk.base.widgets.simpleqt import QtAll
from dsk.base.lib.default_path import DefaultPath



from dsk.base.conn_helper.authentication.web_login_support import get_shotgun_authenticator_support_web_login

from dsk.base.conn_helper.authentication.session_cache import SessionData
from dsk.base.conn_helper.authentication import login
from dsk.base.conn_helper.authentication.sso_saml2.sso_saml2_toolkit import SsoSaml2Toolkit

from dsk.base.conn_helper.authentication.errors import AuthenticationError
from dsk.base.conn_helper.authentication.sso_saml2.core.errors import SsoSaml2MissingQtModuleError

from dsk.base.conn_helper.authentication.sso_saml2.utils import is_autodesk_identity_enabled_on_site
from dsk.base.conn_helper.authentication.sso_saml2.utils import is_sso_enabled_on_site
from dsk.base.conn_helper.authentication.sso_saml2.utils import is_unified_login_flow_enabled_on_site


from dsk.base.conn_helper.authentication.widgets.recent_box import RecentBox
from dsk.base.conn_helper.authentication.widgets.aspect_preserving_label import AspectPreservingLabel
from dsk.base.conn_helper.authentication.widgets.qt5_like_line_edit import Qt5LikeLineEdit
...
class QuerySiteAndUpdateUITask(QtAll.QtCore.QThread):
    """
    ...
    def __init__(self, parent, http_proxy=None):
        ...
        Constructor.
        """
        QtAll.QtCore.QThread.__init__(self, parent)
        self._url_to_test = ""
...
class LoginAuthDialog(QtAll.QtWidgets.QDialog):
    """
    ...
    def __init__(self,
        ...
        # setup the gui
        self.ui = QtAll.uic.loadUi(DefaultPath.getUiFile("login_auth_dialog"), self)
        ...
        sizePolicy = QtAll.QtWidgets.QSizePolicy(QtAll.QtWidgets.QSizePolicy.Preferred, 
                                                 QtAll.QtWidgets.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.ui.logo.sizePolicy().hasHeightForWidth())
        ...
        self.ui.credential_backup_label.setPixmap(QtAll.QtGui.QPixmap(backup_path))
        ...
        if fixed_host:
            self._disable_text_widget(self.ui.site,
        ...
        if is_session_renewal:
            ...
            self._disable_text_widget(self.ui.login,
    ...
    def _populate_user_dropdown(self, site):
        ...
        """
        if site and self._session:
            users = self._session.get_recent_users(site)
    ...
    def _link_activated(self, site):
        ...
        if not QtAll.QtGui.QDesktopServices.openUrl(forgot_password):
    ...
    def _toggle_web(self):
        ...
        if self._use_web != use_web:
            ...
            if self._use_web:
                ...
                self.ui.site.setFocus(QtAll.QtCore.Qt.OtherFocusReason)
            else:
    ...
    def _current_page_changed(self, index):
        ...
        """
        if self.ui.stackedWidget.indexOf(self.ui.fa_page) == index:
    ...
    def exec_(self):
        ...
        #if QtAll.QtCore.__version__.startswith("4."):
        if QtAll.which == 'PyQt4':
            self.show()
        ...
        # other flags.
        self.setWindowFlags(QtAll.QtCore.Qt.WindowStaysOnTopHint)
        return QtAll.QtWidgets.QDialog.exec_(self)
    ...
    def result(self):
        ...
        if self._session_metadata and self._sso_saml2:
            ...
            if res == QtAll.QtWidgets.QDialog.Accepted:
    ...
    def _ok_reject(self):
        """
        """
        super(LoginAuthDialog,self).reject()
    ...
    def _ok_pressed(self):
        ...
        QtAll.QtWidgets.QApplication.setOverrideCursor(QtAll.QtCore.Qt.WaitCursor)
        try:
            if not self._query_task.wait(THREAD_WAIT_TIMEOUT_MS):
                logger.warning("Timed out awaiting check for SSO support on the site: %s" % self._get_current_site())
        finally:
            QtAll.QtWidgets.QApplication.restoreOverrideCursor()
    ...
    def _authenticate(self, error_label, site, login, password, auth_code=None):
        ...
        try:
            if self._use_web and self._sso_saml2:
                ...
                # set the wait cursor
                QtAll.QtWidgets.QApplication.setOverrideCursor(QtAll.QtCore.Qt.WaitCursor)
                QtAll.QtWidgets.QApplication.processEvents()

                # try and authenticate
                self._new_session_token = SessionData.generate_session_token(site,
                                                                             login,
                                                                             password,
                                                                             self._http_proxy,
                                                                             auth_code)
    ...
    def _verify_2fa_pressed(self):
        ...
        """
        self._verify_pressed(self.ui.fa_code.text(), self.ui.invalid_code)
