...
if QtT.which in ['PySide2','PyQt5']:
    ...
    Qt5LikeLineEdit = QtT.QtWidgets.QLineEdit
else:

    class Qt5LikeLineEdit(QtT.QtWidgets.QLineEdit):
        """
        ...
        def paintEvent(self, paint_event):
            ...
            QtT.QtWidgets.QLineEdit.paintEvent(self, paint_event)
            ...
            if self.hasFocus() and not self.text() and self.placeholderText():
                ...
                p = QtT.QtWidgets.QPainter(self)
                pal = self.palette()

                panel = QtT.QtWidgets.QStyleOptionFrameV2()
                self.initStyleOption(panel)
                r = self.style().subElementRect(QtT.QtWidgets.QStyle.SE_LineEditContents, panel, self)
                ...
                visual_alignment = QtT.QtWidgets.QStyle.visualAlignment(self.layoutDirection(), QtT.QtCore.Qt.AlignLeft)
                vertical_alignment = visual_alignment & QtT.QtCore.Qt.AlignVertical_Mask

                if vertical_alignment == QtT.QtCore.Qt.AlignBottom:
                    vscroll = r.y() + r.height() - fm.height() - self._vertical_margin
                elif vertical_alignment == QtT.QtCore.Qt.AlignTop:
                    vscroll = r.y() + self._vertical_margin
                else:
                    vscroll = r.y() + (r.height() - fm.height() + 1) / 2

                line_rect = QtT.QtCore.QRect(
                ...
                elided_text = fm.elidedText(self.placeholderText(), QtT.QtCore.Qt.ElideRight, line_rect.width())
