...
from dsk.base.conn_helper.authentication.sso_saml2.utils import has_sso_info_in_cookies
from dsk.base.conn_helper.authentication.sso_saml2.utils import has_unified_login_flow_info_in_cookies

from dsk.base.conn_helper.authentication import interactive_authentication
from dsk.base.conn_helper.authentication import user
from dsk.base.conn_helper.authentication import user_impl

from dsk.base.conn_helper.authentication.errors import IncompleteCredentials
from dsk.base.conn_helper.authentication.defaults_manager import DefaultsManager
...
class ShotgunAuthenticator(object):
    ...
    def clear_default_user(self):
        ...
        """
        try:
            user = self.create_session_user(host=self._defaults_manager.get_host(),
                                            login=self._defaults_manager.get_login(),
                                            http_proxy=self._defaults_manager.get_http_proxy())
            self._defaults_manager.default_location().delete_session_data(user.host, 
                                                                          user.login)
            return user
    ...
    def get_user_from_prompt(self):
        ...
        host, login, session_token, session_metadata = interactive_authentication.authenticate(
                                                                ...
                                                                self._defaults_manager.default_location())

        return self._create_session_user(login=login,
                                         session_token=session_token,
                                         host=host,
                                         http_proxy=self._defaults_manager.get_http_proxy(),
                                         session_metadata=session_metadata)

    def _create_session_user(self, 
                             login, 
                             session_token=None, 
        ...
        impl = user_impl.SessionUser(host, 
                                     login, 
                                     session_token, 
                                     http_proxy, 
                                     password=password, 
                                     session_metadata=session_metadata,
                                     session=self._defaults_manager.default_location())
    ...
    def create_script_user(self, api_script, api_key, host=None, http_proxy=None):
        ...
        """
        return user.ShotgunUser(user_impl.ScriptUser(host or self._defaults_manager.get_host(),
                                                     api_script,
                                                     api_key,
    ...
    def get_default_user(self):
        ...
        if "api_script" in credentials or "api_key" in credentials:
            ...
            return self.create_script_user(api_script=credentials.get("api_script"),
        ...
        # will take care of it.
        elif ("login" in credentials or 
              "password" in credentials or 
              "session_token" in credentials):

            return self._create_session_user(login=credentials.get("login"),
        ...
        # We don't know what this is, abort!
            ...
            raise IncompleteCredentials("unknown credentials configuration: %s" % credentials)
