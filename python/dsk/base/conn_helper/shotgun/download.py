...
from dsk.base.resources.dsk_errors import ShotgunAttachmentDownloadError
from dsk.base.resources.dsk_errors import DskError
from dsk.base.utils.filesystem_utils import FileSystemUtils as filesystem

from dsk.base.utils.zip_utils import unzip_file
...
def download_url(sg, url, location, use_url_extension=False):
    ...
    try:
        ...
        raise DskError("Could not download contents of url '%s'. Error reported: %s" % (url, e))
...
def download_and_unpack_attachment(sg, attachment_id, target, retries=5, auto_detect_bundle=False):
    ...
    while not invalid_zip_file and not done and attempt < retries:
        ...
        zip_tmp = os.path.join(tempfile.gettempdir(), "%s_sgtk.zip" % uuid.uuid4().hex)
        try:
            ...
            log.warning("Attempt %s: Attachment download of id %s from %s failed: %s" % (attempt, attachment_id, sg.base_url, e))
            attempt += 1
