...
from dsk.base.resources.dsk_errors import UnresolvableCoreConfigurationError
...
def __get_api_core_config_location():
    ...
    this operation is guaranteed to work on any valid sgtk installation
...
def get_project_name_studio_hook_location():
    ...
    core_cfg = __get_api_core_config_location()
    path = os.path.join(core_cfg, dsk_constants.STUDIO_HOOK_PROJECT_NAME)
    return path
...
def __get_sg_config_data(shotgun_cfg_path, user="default"):
    ...
    try:
        ...
        raise DskError("Cannot load config file '%s'. Error: %s" % (shotgun_cfg_path, error))
...
def __get_sg_config_data_with_script_user(shotgun_cfg_path, user="default"):
    ...
    if config_data.get("api_script") and config_data.get("api_key"):
        ...
        raise DskError("Missing required script user in config '%s'" % shotgun_cfg_path)
...
def __sanitize_url(server_url):
    ...
    tokens_parsed = urlparse(server_url)
    ...
    clean_url_tokens = ParseResult(scheme=tokens_parsed.scheme or "https",
                                            netloc=tokens_parsed.netloc or tokens_parsed.path,
                                            path="", params="", query="", fragment="")

    return urlunparse(clean_url_tokens)
...
def create_sg_connection(user="default"):
    ...
    if sg_user is None:
        ...
        try:
            ...
            raise DskError("Cannot connect to Shotgun - this tk session does not have "
