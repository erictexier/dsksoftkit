...
from dsk.base.lib.msg_arg import MsgAssetTypeChange

from dsk.base.resources.browser_constant import  WINDOW_NAME_ASSETTYPE


class AssetTypeWidgetPref(GenTree):
    def __init__(self):
        super(AssetTypeWidgetPref,self).__init__()
        self.assettypeOnOff = dict()


    def set_assettype(self,assetype,value = True):
        self.assettypeOnOff[assetype] = value

    def get_assettype(self,assetype):
...
class AssetTypeWidget(QtT.QtWidgets.QWidget):
    #######################
    def __init__(self, parent, **parameter):
        super(AssetTypeWidget, self).__init__(parent)
        widgetname = parameter.get('object_name', WINDOW_NAME_ASSETTYPE)
        self._isradio = False
    ...
    def __actionSignal(self,act):
        ...
        act(self,confsig.CHANGE_ASSET_PREF_TYPE.name, True, self.refresh)
    ...
    def initFirstTime(self, msg):
        ...
        if globState.has(myName):
            ...
            # add myPref
            self._myPref = globState[myName] = AssetTypeWidgetPref()
        ...
        for x in astype:
            ...
            if is_new == True:
                ...
                if x.active == True and firstfound == False:
                    vv = True
                    firstfound = True
                self._myPref.set_assettype(x.label, vv)
                x.active = vv
            else:
                x.active = self._myPref.get_assettype(x.label)
    ...
    def refresh(self,msg):
        if msg.succeed():
            self._myPref.update_data(msg.get_assettype_change())
            self.update_header()
    ...
    def toggle_value_astype(self,val):
        ...
        msg = MsgAssetTypeChange(self,None,res)
        self.sig[confsig.CHANGE_ASSET_PREF_TYPE.name].emit(msg)
