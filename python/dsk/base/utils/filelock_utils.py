...
def lock_file(path, preserveTime=True):
    ...
    try:
        ...
        raise DskFileErr(exc.returncode, "lock failed: could not %s" %os.path.basename(exc.cmd[0]), path)
    return 0
...
class FilePermissions(object):
    ...
    def _curstat(self, path):
        ...
        fkind = '?'
        if stat.S_ISDIR(info.st_mode):   fkind = 'D'
        elif stat.S_ISREG(info.st_mode): fkind = 'F'
        elif stat.S_ISLNK(info.st_mode): fkind = 'L'
        return (fkind, stat.S_IMODE(info.st_mode), uname, gname, uid, gid, path)
    ...
    def lock_file(self, path, chowned_set=None, utime=False):
        ...
        # The shared set from the current session should be passed in, to keep track of which files
        # have been chown'd to in watchman case of a rollback. This is a keyword argument to stay compatible
        # with spk.
