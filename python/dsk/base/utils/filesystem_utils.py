"""
Utility methods for manipulating files and folders with umask
"""
...
class FileSystemUtils(object):
    ...
    def touch_file(path, permissions=0o666):
        ...
        :param permissions: Optional permissions to set on the file.
                            ...
                            creating a file that is readable and writable
                            for all users.
        ...
        if not os.path.exists(path):
            try:
                ...
                # storage setups so make sure that we ignore any file already
                # exists errors, as they are not really errors!
    ...
    def ensure_folder_exists(
                            path,
                            permissions=0o775, create_placeholder_file=False):
        """
        Helper method - creates a folder and parent folders if such do not
                        already exist.

        :param path: path to create
        :param permissions: Permissions to use when folder is created
        :param create_placeholder_file: If true, a placeholder file will be
               generated.
            ...
            try:
                ...
                if create_placeholder_file:
                    ...
                    if not os.path.exists(ph_path):
                        fh = open(ph_path, "wt")
                        fh.write("This file was generated by Dsk.\n\n")
                        fh.write("The placeholder file is needed when")
                        fh.write("managing toolkit configurations\n")
                        fh.write("in source control packages such as git")
                        fh.write("and perforce. These systems do not \n")
                        fh.write("handle empty folders so a placeholder")
                        fh.write("file is required for the folder \n")
                        fh.write("to be tracked and managed properly.\n")
                        fh.close()
    ...
    def copy_file(src, dst, permissions=0o666):
        ...
        :param permissions: Permissions to use for target file. Default
                            permissions will be readable and writable
                            for all users.
        """
        shutil.copy(src, dst)
    ...
    def move_folder(cls,
        ...
        if os.path.exists(src):
            ...
            src_files = cls.copy_folder_nocheck(src,
                                                dst,
                                                folder_permissions)
    ...
    @classmethod
    @with_cleared_umask
    def backup_folder(cls, src, dst=None):
        """
        ...
        - ``backup_folder("/foo/bar")`` will move ``/foo/bar`` to
          ``/foo/bar.20160912_200426``
        - ``backup_folder("/foo/bar", "/tmp")`` will move ``/foo/bar`` to
          ``/tmp/bar.20160912_200426``
        ...
        if os.path.exists(src):
            ...
            if dst is None:
                ...
                backup_path = os.path.join(dst, "%s.%s" % (
                                        os.path.basename(src), timestamp))
            cls.move_folder(src, backup_path)
    ...
    def create_valid_filename(value):
        ...
        """
        # regex to find non-word characters - in ascii land, that is
        # [^A-Za-z0-9_-.] note that we use a unicode expression,
        # meaning that it will include other "word" characters, not just A-Z.
    ...
    def get_permissions(path):
        ...
        :raises: OSError - if there was a problem retrieving permissions
         for the path
        """
                ...
                try:
                    ...
                    if not (attr & stat.S_IWRITE):
                        ...
                        try:
                            ...
                            log.warning(
                                "Couldn't delete %s: %s. Skipping" % (path, e))
                    else:
                        log.warning("Couldn't delete %s: Skipping" % path)
                except Exception as e:
                    log.warning(
                        "Could not delete %s: %s. Skipping" % (path, e))
            else:
                log.warning("Couldn't delete %s. Skipping." % path)
...
def open_file_browser(path):
    ...
    if os.path.isfile(path):
        ...
        # possibly we have an image sequence and therefore its a
        # ymbolic path, instead check to see if the parent folder of the path
        # is valid and try opening that.
        #
