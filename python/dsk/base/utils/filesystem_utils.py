...
class FileSystemUtils(object):
            ...
            try:
                ...
                if create_placeholder_file:
                    ...
                    if not os.path.exists(ph_path):
                        fh = open(ph_path, "wt")
                        fh.write("This file was automatically generated by Dsk.\n\n")
            ...
            try:
                if os.path.isdir(srcname):
                    if os.path.exists(dstname):
                        try:
                            cls.move_folder(dstname,tempfile.mkdtemp(),rm_dir = True, do_log=False)
                        except Exception as e:
                            log.error("Could not delete directory %s: %s" % (dstname, e))
                            pass # maybe we should raise
                    files.extend(cls.copy_folder(srcname, dstname, folder_permissions))
                else:
    ...
    @classmethod
    @with_cleared_umask
    def copy_folder_nocheck(cls, src, dst, folder_permissions=0o775, skip_list=None):
        """
        Alternative implementation to ``shutil.copytree``
    ...
    @classmethod
    @with_cleared_umask
    def move_folder(cls, src, dst, folder_permissions=0o775, rm_dir = False, do_log = True):
        """Moves a directory.
        ...
        if os.path.exists(src):
            ...
            if rm_dir:
                try:
                    ...
                    if len(all_files) == 0:
                        os.rmdir(src)
                        if do_log == True: log.debug("dir %s: removed" % (src))
                    else:
                        if do_log == True: log.info("no Empty %s" % all_files)
                except Exception as e:
