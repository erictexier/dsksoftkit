...
def clean_path(path):
    ...
    return dirmap.get_link_remap(abspath).rstrip('/')
    # convert /.mounts/ back to /mnt/studio/ links
...
def get_mode(path,new_dir=False,lock_prod=False):
    '''
    get mode for LOCKING directories and initial creation of directories
    (eg making shot dir or making asset dirs)
        
    in prod tree:
        down to $NCO_PROJECT_PATH/prod/shots/<SEQUENCE>/<SHOT>/<DEPT> 0755. 
        these will always be directories
        down to $NCO_PROJECT_PATH/prod/assets/<ASSET_TYPE>/<ASSET_NAME>/
                                 <DEPT> 0755. these will always be directories

        initial creation:
            $NCO_PROJECT_PATH/prod/shots/<SEQUENCE>/<SHOT>/<DEPT>/common
                           and below 0775 (directories) and 0664 (files)
            $NCO_PROJECT_PATH/prod/assets/<ASSET_TYPE>/<ASSET_NAME>
                /<DEPT>/common and below 0775 (directories) and 0664 (files)
        in place publishing:
            $NCO_PROJECT_PATH/prod/shots/<SEQUENCE>/<SHOT>/<DEPT>/common
                          and below 0755 (directories) and 0644 (files)
            $NCO_PROJECT_PATH/prod/assets/<ASSET_TYPE>/<ASSET_NAME>
            /<DEPT>/common and below 0755 (directories) and 0644 (files)

        except pikit library:
            $NCO_PROJECT_PATH/prod/assets/pikit and below 0755 (directories)
                  and 0644 (files)

    in publish tree:
        down to $NCO_PROJECT_PATH/publish/shots/<SEQUENCE> 0755. these
                              will always be directories
        down to $NCO_PROJECT_PATH/publish/assets/<ASSET_TYPE> 0755.
                                           these will always be directories

        from $NCO_PROJECT_PATH/publish/shots/<SEQUENCE>/<SHOT> 1775
                                          (directories) and 0644 (files)
        from $NCO_PROJECT_PATH/publish/assets/<ASSET_TYPE>/<ASSET_NAME> 1775
                                             (directories) and 0644 (files)
    ...
    if os.path.isdir(path):
        ...
        for root,dirs,files in os.walk(path):
            for d in dirs:
                ...
                sys.stdout.write(
                    ">>>>file_utils chmod_recursive dir %s mode %s\n" % (
                                                    this_dir,mode))
                sys.stdout.flush()
            ...
            for f in files:
                ...
                sys.stdout.write(
                    ">>>>>file_utils chmod_recursive file %s mode %s\n" % (
                                                    this_file,mode))
                sys.stdout.flush()
...
def ln(target,linkname):
    sys.stdout.write(
        ">>>>>>>>>>file_utils ln target %s linkname %s\n" % (target,linkname))
    sys.stdout.flush()

    cmd = [os.path.join(show_config.get(
        "publish_user:bin", "/bin"),'wm_ln'),'-sfn',target,linkname]
    return subprocess.check_call(cmd)
...
def lock_file(path, preserveTime=True):
    ...
    # which will make wm_cp
    # unable to read the file. watchman should be in the same group as users,
    # so attempt to make files
    ...
    try:
        ...
        raise FileUtilsErr(exc.returncode,
                           "lock failed: could not %s" % os.path.basename(
                                            exc.cmd[0]), path)
    return 0
...
def lock_dir_recursive(path, preserveTime=True):
    ...
    try:
        ...
        raise FileUtilsErr(exc.returncode,
                           "lock failed: could not %s: %s" % (
                               os.path.basename(exc.cmd[0]), exc), path)
    return 0
...
def get_latest_file_version(filepath_pattern, version_pattern=None, dirs=None):
    ...
    @param filepath_pattern A path pattern used to search for versions.
    ...
    Use the pattern:
    "T:/publish/nut_job/flo/prod/q998/s0030/wip/tnj_q998_s0030_flo_wip_*.ma"
    @param version_pattern A version pattern used to extract the version
        information
    from the filename. This must include a regex group for the version number.
    The
    default is "v(\d{3})" will find the version pattern "v###"
    @returns a list of version numbers in ascending order.
    """
    versions = get_file_versions(filepath_pattern,
                             version_pattern=version_pattern, dirs=dirs)
    if versions:
        return versions[-1]
    return None
...
def split_versioned_file(filepath, version_pattern=None):
    """
    Split a versioned file into three parts: the prefix leading up to the vers,
    ...
    @param version_pattern The regex pattern used to split the filepath.
    It must include
    groups for prefix, version and suffix. If set to None, the routine
    defaults to "(.*)(v\d{3})(.*)".
    @returns a dictionary containing the keys "prefix", "version" and 
        "suffix". If unable to split the filepath, None is returned.
    """
    version_pattern = version_pattern or "(.*)(v\d{3})(.*)"
...
def get_abs_symlinked_file(file_path):
    '''
    Given a file path to a file, return the absolute symlinked target path
    of the file (i.e. the absolute path to the
    file that the link points to).  If it is not a symlinked file, just
    return the absolute path to the file.
    @param filepath The full file path to the file
    @return The real path to the given file
    '''
    if os.path.islink(file_path):
        file_path = os.path.join(os.path.dirname(os.path.abspath(file_path)),
                                    os.path.basename(os.readlink(file_path)))
    else:
        file_path = os.path.join(os.path.dirname(os.path.abspath(file_path)),
...
def is_gzip(filepath):
    """
    Test if a file is gzip compressed or not. Reads the first two byte of
    the file and see if they contain they gzip magic number '\x1f\x8b'.
    """
