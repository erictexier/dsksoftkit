...
class LogUtils(object):
    ...
    def clean(self):
        ...
        #ogging.shutdown(self._handlerObjectList)
    ...
    def _startLog(self,logTag):
        ...
        # format can be set for each handler separately
        self.setFormat()

        # init logging level
        # same than format, can be  specify for each handler later
        self.setLogLevel(self._loglevel)
        # tell the MsgUtils about it
    ...
    def startLogConsole(self,logTag):
        ...
        """ this is a basic log not using logging to redirect the output to a file
        """
        self.clean()
    ...
    def pushLogs(filename,maxLogFile):
        ...
        DiskUtils.rename_file(os.path.join(adir,basefile),os.path.join(adir,'%s.1' % basefile))
...
if __name__ == '__main__':
    ...
    #alog.addHandlerStream() # if run will keep print during the logging to file Time
    ...
    for i in range(5):printSample("AFTERREMOVE")
